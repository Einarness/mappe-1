...
# Henter liste [1] fra df og gjør den om til en tibble som jeg kaller for bnp
head(df[1])
bnp <- df[[1]] 

 
# Endrer type på årtallene til heltall,<int>
bnp <- bnp %>% 
  mutate(år = as.integer(år)) %>% 
# Endrer navn på eksisterende variabler 
...
bnp%>%
# Lager en ny variabel "pertusen" 
  mutate(pertusen = verdi/1000) %>%
 
# Velger verdimengden til variablen tid
  filter(tid >= 2000)%>%

# Bruker ggplot for å lage figuren
  ggplot(aes(x=tid, y=pertusen, colour = var)) + 
  geom_line()+
  labs(title="BNP I Kroner Per Person, I Både Løpende og faste priser",subtitle="Hvordan har inntektene utviklet seg?", x="tid",y="tusen kr", colour="BNP") + 
  theme_fivethirtyeight() + 
  theme(axis.title=element_text())

```

## Oppgave 1b: Skriv en tydelig forklaring og tolkning av figuren. Hvordan har inntektene utviklet seg? Forklar forskjellen mellom BNP i løpende og faste priser. Til hvilke formål er de mest relevante?

Figuren viser BNP i tusener av kroner per person, i både løpende og faste priser, mellom 2000 og 2021. X-aksen viser tida(år) etter 2000, mens y-aksen verdien i tusen kr. Den røde grafen viser utviklingen i bruttonasjonalprodukt i løpende priser, altså nominell BNP. Den ser vi stige jevnt og trutt oppover, bortsett fra noen år med nedgang. Den blå grafen viser utviklingen i BNP i faste 2015-priser. Den viser den virkelige endringen i BNP, altså reell BNP. Vi kan se at den reelle endringen i BNP har økt svakt fra 2000 til 2021, med noen år med nedgang.

Grafen forteller oss blant annet, noe om inntektene i landet, volumendringer, den økonomiske veksten og prisendringene. Nominell BNP er den samlede markedsverdien av den økonomiske produksjonen som ble produsert geografisk i det landet i løpet av et år, uten effekten av inflasjon eller deflasjon, med dagens prisnivå. Den forteller oss hvordan inntektene til landet er og utvikler seg. Vi ser at inntektene til landet, den røde grafen, har økt relativt jevnt fra 2000 til 2021, med noen perioder med nedgang. Det vi også kan tolke ut ifra grafen er hvordan økonomien har utviklet seg. Reell BNP, den blå grafen, er en god indikator på økonomisk vekst, siden det representerer verdien av det som produseres, etter å ha tatt høyde for inflasjon eller deflasjon. Vi ser at økonomien i landet har totalt sett vokst i perioden 2000-2021, men man har hatt perioder der utviklingen har vært negativ. Et annet element vi kan tolke ut ifra figuren er at prisveksten har vært stor. Dette ser vi ved å sammenligne den røde og den blå grafen. Den røde grafen, som forteller oss BNP i løpende priser, har steget mye i perioden, mens den blå grafen, som forteller oss BNP i faste priser, har beveget seg relativt flatt. Siden den blå grafen forteller oss at volumendringer i produksjonen ikke har vært betydelige, vil bakgrunnen for den store økningen i nominell BNP (den røde grafen) i stor grad skyldes vekst i markedspriser.

Vi kan se at de to grafene krysser hverandre i 2012, 2015 og enda en gang i 2016. Dette indikerer at markedsprisene har gått litt opp og ned i den perioden. I 2012, passerte BNP i løpende priser, BNP i faste 2015-priser. Dette kan tolkes som at prisene i 2012 og 2015 var omtrent lik, og siden endringen i reell BNP var relativ flat, var verdien per person lik.

Forskjellen mellom BNP i løpende og faste priser er at, BNP i faste priser tar bort effekten av prisendringer over tid, tillegg til at det skiller ut volumendringer mellom tidsperioder, mens BNP i løpende priser ikke tar bort effekten av prisendringer. En internasjonal sammenligning bruker BNP per person i faste priser (reell BNP per person), justert for kjøpekraft. Sammenligning av ulike kvartaler i samme regnskapsår, kan også gjøre ved BNP i løpende priser. Ved å se på BNP i faste priser vil man kunne sammenligne ulike regnskapsår og se på for eksempel produksjon og produksjonskostnader. I tillegg vil BNP i faste priser vise det faktiske bildet av landets økonomiske vekst.
...
## Oppgave 2a: Beskriv konkret hva koden gjorde. Sammenlign df (bnp) og df_wide (bnp_wide).

Koden lagde en ny, kortere og bredere dataframe. Når koden kjøres blir de to ulike observasjonene (Bruttonasjonalprodukt og MEMO: Bruttonasjonalprodukt. Faste 2015-priser) i kolonnen "var", fordelt i to separate kolonner.

Forskjellen mellom "bnp" (df) og "bnp_wide" (df_wide) er at "bnp" inneholder de to observasjonene i samme kolonne. "Bnp_wide" har separert de to observasjonene og laget en bredere dataframe. I tillegg ser vi at i "bnp" er variabelen "verdi" lagt i en egen kolonne, mens i "bnp_wide" er verdien til de to observasjonene separert og lagt i hver sin kolonne.
...
# Lager to nye varibler DBNP, som er reell endring i BNP med løpende priser, og DMEMOBNP, som er reell endring i BNP med faste 2015-priser.
...
# Endrer plassering på vairablen MEMOBNP, slik at kolonnene BNP og LBNP er ved siden av hverandre
bnp_wide <- bnp_wide %>% 
  relocate("MEMOBNP", .before= "LBNP")

# Bruker pivot_longer for å få de to observasjonene "BNP" og "MEMOBNP" under kolonnen/ varabelen "var"
bnp_long <- bnp_wide %>% 
...
# Endret navn på DBNP og DMEMOBNP for å få to mer betydningsfulle variabler i figuren
bnp_wide <- bnp_wide %>% 
  rename(Nominell= DBNP) %>% 
  rename(Reell= DMEMOBNP)

# Endrer plassering på vairablen MEMOBNP, slik at kolonnene Nominell og Reell er ved siden av hverandre
bnp_wide <- bnp_wide %>% 
  relocate("Nominell", .before = "Reell")

# Bruker pivot_longer for å få to observasjoner "Nominell" og "Reell" under kolonnen/ variablen "var"
bnp_wide <- bnp_wide %>% 
  pivot_longer(6:7, names_to = "var", values_to = "verdi")

# Lager figuren ved bruk av ggplot
bnp_wide %>% 
  ggplot(aes(x = tid, y = verdi, colour = var)) + 
  geom_point() +
  geom_line() +
  geom_smooth() + 
  labs(title="Prosentvis Vekst I Nominell Og Reell BNP Per Person", subtitle="Hva skyldes obervasjonene med negativ vekt i reell BNP?", x= "Tid", y="Prosentvis Vekst", colour="BNP") + 
  theme_fivethirtyeight() + 
  theme(axis.title=element_text())

```

## Oppgave 2d: Finnes det observasjoner med negativ vekst i reell BNP? Hva skyldes dette?

Det finnes flere observasjoner med negativ vekst i reell BNP både med utgangspunkt i løpende priser og faste 2015-priser. Dette skyldes at produksjonen i landet har gått ned som følge av nedgang i finansmarkedet. Vi ser at det var negativ vekst i reell BNP i 1987, som skyldes et aksjekrakk vi hadde i Norge. Dette førte til at den finansielle aktiviteten stoppet delvis opp og volumendringen i produksjonen falt. Vi hadde også negativ vekst i reell BNP i 2008-2011, som skyldes en global resesjon i forbindelse med finanskrisen. Dette førte til at finansmarkedet falt enormt, noe som rammet Norge, som produserte mindre varer og tjenester i denne perioden. Den siste observasjonen med negativ vekst i reell BNP var i 2020. Dette skyldes en global pandemi som gjorde at hele verdensøkonomien stoppet opp. Den totale produksjonen av varer og tjenester falt, siden landet ikke hadde samme behov som før pandemien.
